...
CREATE OR REPLACE PACKAGE BODY SLIDING_PUZZLE AS
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE(
            'Время игры: ' ||
            TO_CHAR(EXTRACT(DAY FROM V_ELAPSED) * 24 + EXTRACT(HOUR FROM V_ELAPSED)) || 'ч ' ||
            TO_CHAR(EXTRACT(MINUTE FROM V_ELAPSED)) || 'м ' ||
            TO_CHAR(TRUNC(EXTRACT(SECOND FROM V_ELAPSED))) || 'с'
        );
    END GAME_OVER;
    ...
    BEGIN
        ...
        OPEN V_CURSOR FOR
            SELECT 
                ...
                PS.GRID_SIZE,
                ROUND((CAST(SYSTIMESTAMP AS DATE) - CAST(GA.STARTED_AT AS DATE)) * 24 * 60) AS ELAPSED_MINUTES,
                EXTRACT(MINUTE FROM GA.TIME_LIMIT) +
                EXTRACT(HOUR FROM GA.TIME_LIMIT) * 60 +
                EXTRACT(DAY FROM GA.TIME_LIMIT) * 1440 AS TIME_LIMIT_MINUTES
            FROM GAME_ATTEMPTS GA
            JOIN PUZZLES PZ ON GA.PUZZLE_ID = PZ.ID
            JOIN PUZZLE_SIZES PS ON PZ.PUZZLE_SIZE_ID = PS.ID
            WHERE GA.ID = V_ATTEMPT_ID;
    ...
    BEGIN
        OPEN V_CURSOR FOR
            SELECT 
                ...
                DL.SHUFFLE_MOVES,
                PS.GRID_SIZE,
                EXTRACT(DAY FROM PS.DEFAULT_TIME_LIMIT) * 1440 +
    ...
    BEGIN
        ...
        OPEN V_CURSOR FOR
            SELECT 
                ...
                PS.GRID_SIZE,
                DL.NAME AS DIFFICULTY,
                ROUND((GS.END_TIME - GS.START_TIME) * 24 * 60) AS DURATION_MINUTES
            FROM GAME_SESSIONS GS
            JOIN GAME_STATUSES GST ON GS.STATUS_ID = GST.ID
            JOIN PUZZLES PZ ON GS.PUZZLE_ID = PZ.ID
            JOIN PUZZLE_SIZES PS ON PZ.PUZZLE_SIZE_ID = PS.ID
